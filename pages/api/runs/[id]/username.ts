...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  console.log(`👤 [${new Date().toISOString()}] ${req.method} /api/runs/${req.query.id}/username`);
  ...
  try {
    ...
    console.log('📝 Username update request:', { runId, username });
    ...
    console.log('✅ Run found:', { id: run.id, currentUsername: run.username });
    ...
    if (hoursSinceCreation > 24) {
      console.log('❌ Run too old to update:', { hoursSinceCreation });
    ...
    console.error('💥 API Error in username update:', error);
